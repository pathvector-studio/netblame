...
「ネットが遅い」「サイトに繋がらない」— そのとき本当に悪いのは、ルータなのか、DNS なのか、途中の経路なのか、それともサーバなのか。`blame` は URL / ホストを 1 つ渡すだけで段階診断を行い、**最も可能性の高い犯人を平易な日本語で名指し**するシングルバイナリの CLI です。

```
$ blame https://example.com
...
...
Rust ツールチェーン (1.85+) があればソースからビルドできます。

```bash
git clone <this repo>
cd blame
cargo build --release
# バイナリは target/release/blame
```
...
```
$ blame https://example.com
blame: example.com (port 443) を診断します…
...
```
$ blame https://definitely-not-a-real-domain-xyz123.com
...
```
$ blame example.com:81 --timeout 2
...
```
$ blame https://expired.badssl.com --samples 2
...
- **traceroute / MTU プローブ**: 現状は「経路のどこか」までしか言えない。ICMP/UDP traceroute で「宅内 or ISP or 対岸」を切り分け、DF ビット付きプローブで PMTUD 黒穴 (VPN あるある) を検出したい。raw socket 権限 (CAP_NET_RAW) の扱いが課題
- **QUIC/HTTP3 対応**: UDP 443 がブロックされて HTTP/3 だけ遅い、という新種の事故の検出
- **継続監視モード**: `blame --watch` で断続的な問題を捕まえ、発生時刻つきのタイムラインを出す
- **レポート共有 SaaS 構想**: `--json` レポートをワンコマンドでアップロードして短縮 URL を発行、「これ見て」と情シス/ISP サポートに投げられるサービス。多数のレポートが集まれば「この ISP のこの地域で今日 DNS が遅い」という群衆診断 (crowdsourced outage map) に発展できる
