...
| `--max-time <secs>` | 診断全体のウォールクロック上限秒数 (curl 流)。`--timeout` はプローブ単位のため、全ステージ合計はその数倍かかりえます — `--max-time` は総時間を打ち切ります。期限を過ぎると残りステージをスキップし、**そこまでの部分レポートを必ず出力**します (テキストは `[TRUNCATED]` バナー、JSON は `report.completeness` に `complete` / `truncated_reason` / `ran_stages` / `skipped_stages`) | 未設定 (無制限) |
...
**終了コード**: `0` = 問題なし / `1` = 問題を検出 / `2` = 使い方・内部エラー / `3` = 判定に至る前に `--max-time` で打ち切り (期限時点で犯人が特定済みなら通常の終了コードを維持)

**ステージ別所要時間**: JSON レポートには常に `report.stage_durations` (ステージ別ミリ秒) が含まれ、遅い診断がどこで時間を使ったか特定できます — 例: HTTP/3 非対応ホストで QUIC プローブがタイムアウトを丸ごと待つケース。
