...
> ⚠️ **プロトタイプ段階です。** 判定エンジンの閾値はヒューリスティックで、珍しいネットワーク環境では誤判定があり得ます。フィードバック歓迎。現在 Linux で動作確認済み(macOS はベストエフォート、Windows は未対応)。
...
- **継続監視モード**: `netblame --watch` で断続的な問題を捕まえ、発生時刻つきのタイムラインを出す
